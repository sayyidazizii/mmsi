<?php
	...
	class AcctSavingsAccount_model extends CI_Model {
		...
		public function getAcctSavingsPoint($savings_id)
		{
			$this->db->select('savings_point');
			$this->db->from('acct_savings');
			$this->db->where('savings_id', $savings_id);
			$this->db->where('data_state', 0);
			$result = $this->db->get()->row_array();
			return $result['savings_point'];
		}
