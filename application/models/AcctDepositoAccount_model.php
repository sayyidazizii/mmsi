...
class AcctDepositoAccount_model extends CI_Model
{
	...
	public function getDataAcctDepositoAccount($deposito_id, $branch_id){
		$this->db->select('acct_deposito_account.deposito_account_id, acct_deposito_account.member_id, core_member.member_name, acct_deposito_account.deposito_id, acct_deposito.deposito_code, acct_deposito.deposito_name, acct_deposito_account.deposito_account_no, acct_deposito_account.deposito_account_date, acct_deposito_account.deposito_account_amount, acct_deposito_account.deposito_account_due_date, acct_deposito_account.deposito_account_serial_no, acct_deposito_account.deposito_account_nisbah, acct_deposito_account.validation, acct_deposito_account.validation_id, acct_deposito_account.validation_on, acct_deposito_account.deposito_account_blockir_type, acct_deposito_account.deposito_account_blockir_status, acct_deposito_account.deposito_account_cashback, acct_deposito_account.deposito_account_commission_on_hold_agent, acct_deposito_account.deposito_account_commission_disbursed_agent,  acct_deposito_account.deposito_account_commission_on_hold_supervisor, acct_deposito_account.deposito_account_commission_disbursed_supervisor, acct_deposito_account.deposito_account_buffer, acct_deposito_account.deposito_account_condition');
	...
	public function getAcctDepositoAccountUniqueCode($today, $uniquecode){
		$this->db->select('deposito_account_unique_code');
		$this->db->from('acct_deposito_account');
		$this->db->where('deposito_account_unique_code', $uniquecode);
		$this->db->where('deposito_account_date', $today);
		$this->db->where('data_state', 0);
		return $this->db->get()->result_array();
	}
	...
	public function getAcctDeposito()
	{
		...
		$this->db->where('acct_deposito.deposito_availability >=',  1);
	...
	public function getAcctDepositoAccountDetail($deposito_account_id)
	{
		$this->db->select('*');
		$this->db->from('acct_deposito_account');
		$this->db->join('acct_savings_account', 'acct_savings_account.savings_account_id = acct_deposito_account.savings_account_id');
	...
	public function getAcctDepositoAccountBO($deposito_account_id){
		$this->db->select('acct_deposito_account.deposito_account_id, acct_deposito_account.office_id, acct_savings_account.savings_id, acct_savings_account.member_id, acct_savings_account.savings_account_id ');
		$this->db->from('acct_deposito_account');
		$this->db->join('core_office', 'core_office.office_id = acct_deposito_account.office_id');
		$this->db->join('acct_savings_account', 'core_office.savings_account_id = acct_savings_account.savings_account_id');
		$this->db->join('acct_savings', 'acct_savings.savings_id = acct_savings_account.savings_id');
	...
	public function getAcctSavingsAccountLastBalance($savings_account_id){
		$this->db->select('acct_savings_account.savings_account_last_balance, acct_savings_account.member_id, acct_savings_account.savings_id, acct_savings_account.office_id, acct_savings_account.savings_account_id, acct_savings_account.savings_account_blockir_amount,acct_savings.account_id');
		$this->db->from('acct_savings_account');
		$this->db->join('acct_savings', 'acct_savings_account.savings_id = acct_savings.savings_id');
		$this->db->where('acct_savings_account.data_state', 0);
	...
	{
		$this->db->select('acct_savings_account.savings_account_id, CONCAT(acct_savings_account.savings_account_no," - ",core_member.member_name) AS savings_account_no, acct_savings_account.savings_id, acct_savings_account.member_id, acct_savings_account.savings_account_last_balance, core_member.member_reference AS member_agent');
		$this->db->from('acct_savings_account');
		$this->db->join('core_member', 'acct_savings_account.member_id = core_member.member_id');
		$this->db->join('acct_savings', 'acct_savings_account.savings_id = acct_savings.savings_id');
		$this->db->where('acct_savings_account.data_state', 0);
	...
	public function getAcctSavingsAccountAgentSPV($member_id){
		$this->db->select('acct_savings_account.savings_account_id, core_member.member_id, CONCAT(acct_savings_account.savings_account_no," - ",core_member.member_name) AS savings_account_no, acct_savings_account.savings_id, acct_savings_account.savings_account_last_balance');
		$this->db->from('acct_savings_account');
		$this->db->join('core_member', 'acct_savings_account.member_id = core_member.member_id');
		// $this->db->join('acct_savings', 'acct_savings_account.savings_id = acct_savings.savings_id');
		$this->db->where('acct_savings_account.savings_id', 30);
	...
	public function getCoreMember_Detail($member_id){
		$this->db->select('core_member.member_id, core_member.branch_id, core_branch.branch_name, core_member.member_no, core_member.member_name, core_member.member_gender, core_member.member_place_of_birth, core_member.member_date_of_birth, core_member.member_address, core_member.province_id, core_province.province_name, core_member.city_id, core_city.city_name, core_member.kecamatan_id, core_kecamatan.kecamatan_name, core_member.member_phone, core_member.member_job, core_member.member_identity, core_member.member_identity_no, core_member.member_postal_code, core_member.member_mother, core_member.member_heir, core_member.member_family_relationship, core_member.member_status, core_member.member_register_date, core_member.member_marital_status, core_member.member_reference');
		$this->db->from('core_member');
	...
	public function getDepositoInterestPeriod($deposito_id){
		$this->db->select('deposito_interest_period');
		$this->db->from('acct_deposito');
		$this->db->where('deposito_id', $deposito_id);
		$result = $this->db->get()->row_array();;
		return $result['deposito_interest_period'];
	}
	...
	public function getSavingsAccountAgent($deposito_account_id){
		...
		$this->db->where('deposito_account_id', $deposito_account_id);
		$result = $this->db->get()->row_array();
		return $result['savings_account_id_agent'];
	}


	// public function getDepositoKomisi($deposito_id){
	// 	$this->db->select('deposito_commission');
	// 	$this->db->from('acct_deposito');
	// 	$this->db->where('deposito_id', $deposito_id);
	// 	$result = $this->db->get()->row_array();
	// 	return $result['deposito_commission'];
	// }

	// public function getDepositoKomisiDitahan($deposito_id){
	// 	$this->db->select('deposito_commission_on_hold');
	// 	$this->db->from('acct_deposito');
	// 	$this->db->where('deposito_id', $deposito_id);
	// 	$result = $this->db->get()->row_array();
	// 	return $result['deposito_commission_on_hold'];
	// }

	// public function getDepositoKomisiDicairkan($deposito_id){
	// 	$this->db->select('deposito_commission_disbursed');
	// 	$this->db->from('acct_deposito');
	// 	$this->db->where('deposito_id', $deposito_id);
	// 	$result = $this->db->get()->row_array();
	// 	return $result['deposito_commission_disbursed'];
	// }

	public function getDepositoAvailability($deposito_id){
		$this->db->select('deposito_availability');
		$this->db->from('acct_deposito');
		$this->db->where('deposito_id', $deposito_id);
		$result = $this->db->get()->row_array();
		return $result['deposito_availability'];
	}
	...
	public function insertAcctDepositoAccount($data_interest_period){
		return $query = $this->db->insert('acct_deposito_account', $data_interest_period);
	}

	public function insertAcctCommission($data){
		return $query = $this->db->insert('acct_commission', $data);
	}

	public function insertTestCron($data){
		return $query = $this->db->insert('core_store', $data);
	}

	// public function insertAcctCommissionOnHold($data_commission_on_hold)
	// {
	// 	if ($this->db->insert('acct_commission_on_hold', $data_commission_on_hold)) {
	// 		return true;
	// 	} else {
	// 		return false;
	// 	}
	...
	public function updateAcctCommission($data_commission){
		$this->db->where("commission_id", $data_commission['commission_id']);
		$query = $this->db->update('acct_commission', $data_commission);
		if ($query) {
			return true;
		} else {
			return false;
	...
	public function processEditCommission($data){
		$this->db->where("commission_id", $data['commission_id']);
		$query = $this->db->update('acct_commission', $data);
		if ($query) {
			return true;
		} else {
			return false;
	...
	public function deleteAcctCommission($commission_id){
		$this->db->where("commission_id", $commission_id);
		$query = $this->db->update('acct_commission', array('data_state'=>1));
		if ($query) {
			return true;
		} else {
			return false;
	...
	public function getListAcctCommission($deposito_account_id){
		$this->db->select('acct_deposito_account.deposito_account_id, acct_deposito_account.deposito_account_no, acct_commission.commission_id, acct_commission.commission_date, acct_commission.commission_on_hold_agent, acct_commission.commission_on_hold_supervisor, acct_commission.commission_disbursed_agent, acct_commission.commission_disbursed_supervisor, acct_commission.commission_disbursed_status');
	...
	public function getAcctCommissionDetail($commission_id){
		$this->db->select('commission_id, commission_date, commission_on_hold_agent, commission_on_hold_supervisor, commission_disbursed_agent, commission_disbursed_supervisor, commission_disbursed_status');
		$this->db->from('acct_commission');
		$this->db->where('commission_id', $commission_id);
		$this->db->where('data_state', 0);
		$result = $this->db->get()->row_array();
	...
	public function getAcctCommission(){
		$this->db->select('*');
		$this->db->from('acct_commission');
		$this->db->join('acct_deposito_account', 'acct_deposito_account.deposito_account_id = acct_commission.deposito_account_id');
	...
	public function getAcctCommissionUpdate(){
		...
		$this->db->where('acct_commission.data_state', 0);
		$date_now = date('Y-m-d');
		$this->db->where('commission_due_date', $date_now);
		return $this->db->get()->result_array();  
	}
	...
	public function getAcctDepositoProfitSharingLast($deposito_account_id){
		$this->db->select('*');
		$this->db->from('acct_deposito_profit_sharing');
		$this->db->where('deposito_account_id', $deposito_account_id);
		$this->db->limit(1);
		$this->db->order_by('deposito_profit_sharing_id', 'DESC');
		$result = $this->db->get()->row_array();
		return $result;
	}
	...
	// public function distributionCommission($data){
	// 	$this->db->where("deposito_account_id", $data['deposito_account_id']);
	// 	$query = $this->db->update($this->table, $data);
	// 	if ($query) {
	// 		return true;
	...
	public function updateAcctDeposito($data){
		...
		$query = $this->db->update('acct_deposito', $data);
		if ($query) {
			return true;
		} else {
			return false;
	...
	public function getAcctDepositoPoint($deposito_id){
		$this->db->select('deposito_point');
		$this->db->from('acct_deposito');
		$this->db->where('deposito_id', $deposito_id);
		$this->db->where('data_state', 0);
		$result = $this->db->get()->row_array();
		return $result['deposito_point'];
	}
