<?php
	...
	class AcctDeposito_model extends CI_Model {
		...
		public function getDataAcctDeposito(){
			$this->db->select('acct_deposito.deposito_id, acct_deposito.deposito_code, acct_deposito.deposito_name, acct_deposito.account_id, acct_account.account_code, acct_account.account_name, acct_deposito.account_basil_id, acct_deposito.deposito_period, acct_deposito.deposito_interest_rate, acct_deposito.deposito_interest_period, acct_deposito.deposito_availability, acct_deposito.deposito_point');
		...
		public function getAcctDepositoInterest($deposito_interest_date, $deposito_id){
			$this->db->select('acct_deposito_interest.deposito_interest_id, acct_deposito_interest.deposito_interest_percentage');
			$this->db->from('acct_deposito_interest');
			$this->db->where('acct_deposito_interest.deposito_interest_date', $deposito_interest_date);
			$this->db->where('acct_deposito_interest.deposito_id', $deposito_id);
			return $this->db->get()->row_array();
		}
		
		public function updateAcctDepositoInterest($data){
			$this->db->where("deposito_interest_date",$data['deposito_interest_date']);
			$this->db->where("deposito_id",$data['deposito_id']);
			$query = $this->db->update('acct_deposito_interest', $data);
			if($query){
				return true;
			}else{
				return false;
		...
		public function insertAcctDepositoInterest($data){
			$query = $this->db->insert('acct_deposito_interest',$data);
			if($query){
				return true;
			}else{
				return false;
		...
		public function insertAcctDepositoAvailability($data_availability){
			$query = $this->db->insert('acct_deposito_availability',$data_availability);
			if($query){
				return true;
			}else{
				return false;
