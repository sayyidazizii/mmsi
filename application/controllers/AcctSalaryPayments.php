<?php
	...
	Class AcctSalaryPayments extends CI_Controller{
		...
		public function addAcctSalaryPayment(){	
			...
			$data['main_view']['acctbankaccount']		= create_double($this->AcctSalaryPayment_model->getAcctBankAccount(),'bank_account_id', 'bank_account_code');	
			
			$data['main_view']['accountcredit']			= $accountcredit;
			$data['main_view']['interest_plus']			= $interest_plus;
			$data['main_view']['detailpayment']			= $this->AcctSalaryPayment_model->getDataByIDCredit($credits_account_id);
				...
				if($credits_payment_token->num_rows() == 0){
					if($this->AcctSalaryPayment_model->insert($data)){
						...
						if($member_mandatory_savings <> 0 || $member_mandatory_savings <> ''){
							...
							$this->AcctSalaryPayment_model->insertAcctSavingsMemberDetail($data_detail);
						}
						...
						$account_id_default_status 			= $this->AcctCreditAccount_model->getAccountIDDefaultStatus($preferencecompany['account_salary_debt_id']);

						$data_debet = array (
							'journal_voucher_id'			=> $journal_voucher_id,
							'account_id'					=> $preferencecompany['account_salary_debt_id'],
							'journal_voucher_description'	=> $data_journal['journal_voucher_title'],
							'journal_voucher_amount'		=> $data['credits_payment_amount'],
							'journal_voucher_debit_amount'	=> $data['credits_payment_amount'],
							'account_id_default_status'		=> $account_id_default_status,
							'account_id_status'				=> 0,
							'journal_voucher_item_token'	=> $data['credits_payment_token'].$preferencecompany['account_salary_debt_id'],
							'created_id' 					=> $auth['user_id'],
						);
						...
						if(!empty($member_mandatory_savings) || $member_mandatory_savings > 0 || $member_mandatory_savings != ''){
							...
							$data_credit =array(
								...
								'journal_voucher_description'	=> 'SETORAN VIA POTONG GAJI '.$AcctSalaryPayment_last['member_name'],
						...
						$member_account_receivable_amount = $this->AcctSalaryPayment_model->getCoreMemberAccountReceivableAmount($data['member_id']);

						$member_account_receivable_amount += $data['credits_payment_amount'];
						...
						echo $AcctSalaryPayment_last['credits_payment_id'];
					}else{
		...
		public function printNoteSalaryPayment(){
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			        ...
			        <td width=\"50%\"><div style=\"text-align: left; font-size:14px\"><br>BUKTI SETORAN ANGSURAN</div></td>
			    </tr>
			    <tr>
		...
		public function indCashLessPayment(){
			...
			$data['main_view']['acctcredits']	= create_double($this->AcctCreditAccount_model->getAcctCredits(),'credits_id', 'credits_name');
			$data['main_view']['corebranch']	= create_double($this->AcctCreditAccount_model->getCoreBranch(),'branch_id', 'branch_name');
			$data['main_view']['content']		= 'AcctSalaryPayment/ListAcctNonCashPayment_view';
			$this->load->view('MainPage_view', $data);
		}
		...
		public function filteracctcashlesspayment(){
			...
			$this->session->set_userdata('filter-AcctSalaryPaymentless', $data);
			redirect('salary-payments/ind-cash-less-payment');
		}

		public function reset_cashless(){
			$this->session->unset_userdata('filter-AcctSalaryPaymentless');
			redirect('salary-payments/ind-cash-less-payment');
		}

		public function getAcctCashLessPayment(){
			$auth 	= $this->session->userdata('auth');
			$sesi	= $this->session->userdata('filter-AcctSalaryPaymentless');
			if(!is_array($sesi)){
				$sesi['start_date']		= date('Y-m-d');
				$sesi['end_date']		= date('Y-m-d');
				$sesi['credits_id']		='';
	        ...
	        foreach ($list as $cashpayment) {
	            ...
	            if($cashpayment->credits_payment_type == 1){
		            ...
		            $row[] = $cashpayment->member_name;
		            $row[] = $cashpayment->credits_name;
		            $row[] = $this->AcctSalaryPayment_model->getSavingsAccountNO($cashpayment->savings_account_id);
		            $row[] = tgltoview($cashpayment->credits_payment_date);
		            $row[] = number_format($cashpayment->credits_payment_principal, 2);
		            $row[] = number_format($cashpayment->credits_payment_interest, 2);
				    $row[] = '<a href="'.base_url().'salary-payments/print-note-cash-less/'.$cashpayment->credits_payment_id.'" class="btn btn-xs blue" role="button"><i class="fa fa-print"></i> Kwitansi</a>';
		            $data[] = $row;
	            }
		...
		public function addCashlessPayment(){	
			...
			$data['main_view']['content']				= 'AcctSalaryPayment/FormAddCashPaymentNon_view';
			$this->load->view('MainPage_view',$data);
		}
		...
		public function simpananlist(){
			...
			foreach ($list as $customers) {
				...
				$row[] = '<a href="'.base_url().'salary-payments/add-cash-less-payment/'.$segment3.'/'.$customers->savings_account_id.'" class="btn btn-info" role="button"><span class="glyphicon glyphicon-ok"></span> Select</a>';
		...
		public function printNoteCashLessPayment(){
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			    	<td rowspan=\"2\" width=\"13%\">".$img."</td>
			        <td width=\"50%\"><div style=\"text-align: left; font-size:14px\"><br>BUKTI SETORAN ANGSURAN NON VIA POTONG GAJI</div></td>
			    </tr>
			    <tr>
			        <td width=\"40%\"><div style=\"text-align: left; font-size:14px\">Jam : ".date('H:i:s')."</div></td>
			    </tr>
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
			    ...
			    <tr>
			        ...
			        <td width=\"50%\"><div style=\"text-align: left;\">: ".$this->AcctSalaryPayment_model->getSavingsAccountNO($acctcreditspayment['savings_account_id'])."</div></td>
			    </tr>
			    <tr>
			        <td width=\"20%\"><div style=\"text-align: left;\">Alamat</div></td>
			        <td width=\"50%\"><div style=\"text-align: left;\">: ".$acctcreditspayment['member_address']."</div></td>
