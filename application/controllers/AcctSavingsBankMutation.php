<?php
	...
	Class AcctSavingsBankMutation extends CI_Controller{
		...
		public function processAddAcctSavingsBankMutation(){
			...
			$savingsaccount					= $this->AcctSavingsBankMutation_model->getAcctSavingsAccount_Detail($data['savings_account_id']);

			$systempointsetting 			= $this->AcctSavingsBankMutation_model->getSystemPointSetting();

			if($this->form_validation->run()==true){
				if($data['savings_bank_mutation_last_balance'] >= $savingsaccount['savings_account_blockir_amount']){
					if($savings_bank_mutation_token->num_rows()==0){
						if($this->AcctSavingsBankMutation_model->insertAcctSavingsBankMutation($data)){
							...
							$this->AcctSavingsBankMutation_model->insertSystemPoint($data_point);
