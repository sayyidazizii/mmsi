<?php
	...
	Class AcctCashPayments extends CI_Controller{
		...
		public function processAddAcctCashPayment(){
			...
			$systempointsetting = $this->AcctCreditAccount_model->getSystemPointSetting();
				...
				if($credits_payment_token->num_rows() == 0){
					if($this->AcctCashPayment_model->insert($data)){
						...
						$data_point = array(
							...
							'point_from' 	=> 'Angsuran Pinjaman',
							'point_amount' 	=> $systempointsetting['point_setting_credits_payment_amount'],
							'branch_id' 	=> $auth['branch_id'],
							'created_id' 	=> $auth['user_id'],
		...
		public function printNoteCashPayment(){
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			        ...
			        <td width=\"40%\"><div style=\"text-align: left; font-size:14px\"><br>BUKTI SETORAN ANGSURAN</div></td>
			    </tr>
			    <tr>
		...
		public function AcctCashPaymentsProcess(){
			...
			if($simpanan['savings_account_last_balance']-$total < $simpanan['savings_account_blockir_amount']){
				$auth 	= $this->session->userdata('auth');
				$msg 	= "<div class='alert alert-danger alert-dismissable'>  
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
								Saldo Akhir Tabungan Kurang dari Saldo Ditahan
							</div> "; 
				$sesi = $this->session->userdata('unique');
				$this->session->unset_userdata('addCashlessPayment-'.$sesi['unique']);
				$this->session->set_userdata('message',$msg);
		...
		public function printNoteCashLessPayment(){
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			    	<td rowspan=\"2\" width=\"13%\">".$img."</td>
			        <td width=\"50%\"><div style=\"text-align: left; font-size:14px\"><br>BUKTI SETORAN ANGSURAN DEBET</div></td>
			    </tr>
			    <tr>
			        <td width=\"40%\"><div style=\"text-align: left; font-size:14px\">Jam : ".date('H:i:s')."</div></td>
			    </tr>
