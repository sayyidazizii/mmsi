...
class AcctCreditsAccountMasterData extends CI_Controller
{
	...
	public function getAcctCreditsAccountMasterDataList()
	{
		...
		foreach ($list as $creditsaccount) {
			...
			$row[] = number_format($creditsaccount->credits_account_special, 2);
			$row[] = $this->AcctCreditsAccountMasterData_model->getStoreName($creditsaccount->store_id);
			$data[] = $row;
		}
	...
	public function exportAcctCreditsAccountMasterData()
	{
		...
		if ($acctcreditsaccountmasterdata->num_rows() != 0) {
			...
			$this->excel->getActiveSheet()->setCellValue('W3', "Biaya Simpanan Wajib Kredit");
			$this->excel->getActiveSheet()->setCellValue('X3', "Toko");
			...
			foreach ($acctcreditsaccountmasterdata->result_array() as $key => $val) {
				if (is_numeric($key)) {
					...
					$this->excel->getActiveSheet()->setCellValue('W' . $j, number_format($val['credits_account_special'], 2));	
					$this->excel->getActiveSheet()->setCellValue('X' . $j, $val['store_name']);	
