<?php
	...
	Class AcctSavingsCashMutation extends CI_Controller{
		...
		public function processAddAcctSavingsCashMutation(){
			...
			$savingsaccount 				= $this->AcctSavingsCashMutation_model->getAcctSavingsAccount_Detail($data['savings_account_id']);

			$systempointsetting				= $this->AcctSavingsCashMutation_model->getSystemPointSetting();

			if($this->form_validation->run()==true){
				if($data['savings_cash_mutation_last_balance'] >= $savingsaccount['savings_account_blockir_amount']){
					if($savings_cash_mutation_token->num_rows()==0){
						if($this->AcctSavingsCashMutation_model->insertAcctSavingsCashMutation($data)){
							...
							$data_point = array(
								...
								'point_from' 	=> 'Mutasi Tabungan',
								'point_amount' 	=> $systempointsetting['point_setting_savings_mutation_amount'],
								'branch_id' 	=> $auth['branch_id'],
								'created_id' 	=> $auth['user_id'],
								'created_on' 	=> date('Y-m-d'),
							);
							$this->AcctSavingsCashMutation_model->insertSystemPoint($data_point);
