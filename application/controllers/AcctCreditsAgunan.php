<?php
	...
	Class AcctCreditsAgunan extends CI_Controller{
		...
		public function getAcctCreditsAgunanList(){
	        ...
	        foreach ($list as $agunan) {
				if($agunan->credits_agunan_type == 1){
					$credits_agunan_type	= 'SK Pengangkatan';
					$credits_agunan_ket		= $agunan->credits_agunan_sk_description;
				}else if($agunan->credits_agunan_type == 2) {
					$credits_agunan_type 	= 'Manulife';
					$credits_agunan_ket		= $agunan->credits_agunan_manulife_description;
				}else if($agunan->credits_agunan_type == 3){
					$credits_agunan_type 	='BPJS Ketenagakerjaan';
					$credits_agunan_ket		= $agunan->credits_agunan_bpjs_description;
				...
				foreach($acctcreditsagunan->result_array() as $key=>$val){
					if(is_numeric($key)){
						...
						if($val['credits_agunan_type'] == 1){
							...
							$desc = $val['credits_agunan_sk_description'];
						}else if($val['credits_agunan_type'] == 2){
							$type = "Manulife";
							$desc = $val['credits_agunan_manulife_description'];
						}else if($val['credits_agunan_type'] == 3){
							$type = "BPJS Ketenagakerjaan";
							$desc = $val['credits_agunan_bpjs_description'];
						}else{
							$type = "Lain - Lain";
		...
		public function printAgunanReceipt(){
			...
			<table id=\"items\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">
				<tr>
					<td style=\"text-align:center;\" width=\"100%\">
						<div style=\"font-size:14px; font-weight:bold\">KOPERASI KARYAWAN RS KASIH IBU SURAKARTA</div>
					</td>			
				</tr>
				<tr>
					<td style=\"text-align:center;\" width=\"100%\">
			...
			<table id=\"items\" width=\"60%\" cellspacing=\"0\" cellpadding=\"5\" border=\"1\">
				<tr>	
					<td style=\"text-align:left;\" width=\"35%\">
						<div style=\"font-size:12px;\">Manulife</div>
					</td>
					<td style=\"text-align:left;\" width=\"75%\">
						<div style=\"font-size:12px;\">".$this->AcctCreditsAgunan_model->getAgunanManulifeDescription(2, $agunandetail['credits_account_id'])."</div>
					</td>			
				</tr>
				<tr>	
					<td style=\"text-align:left;\" width=\"35%\">
						<div style=\"font-size:12px;\">SK</div>
					</td>
					<td style=\"text-align:left;\" width=\"75%\">
						<div style=\"font-size:12px;\">".$this->AcctCreditsAgunan_model->getAgunanSKDescription(1, $agunandetail['credits_account_id'])."</div>
					</td>			
				</tr>
				<tr>	
					<td style=\"text-align:left;\" width=\"35%\">
						<div style=\"font-size:12px;\">BPJS</div>
					</td>
					<td style=\"text-align:left;\" width=\"75%\">
						<div style=\"font-size:12px;\">".$this->AcctCreditsAgunan_model->getAgunanBPJSDescription(3, $agunandetail['credits_account_id'])."</div>
					</td>			
				</tr>
				<tr>	
					<td style=\"text-align:left;\" width=\"35%\">
