...
<script>
	...
	mappia 		= "<?php echo site_url('office/add'); ?>";
	function ulang(){
	...
	function function_elements_add(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('office/elements-add');?>",
    ...
    $(document).ready(function(){
		...
		$('#savings_account_id').combobox({
			onChange: function(value){
				// alert(value);
				var name   	= 'savings_account_id';
	...
	function addCoreDusun(){
		...
		$('#onspinspinwarehouse').css('display', 'table-row');
		  $.ajax({
		  type: "POST",
		  url : "<?php echo site_url('office/add-dusun');?>",
		  data: {
		  			'city_id'			: city_id,
...
<?php echo form_open('office/process-add',array('id' => 'myform', 'class' => 'horizontal-form')); ?>
<?php
	$unique 		= $this->session->userdata('unique');
	$data 			= $this->session->userdata('addCoreOffice-'.$unique['unique']);
...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="<?php echo base_url();?>office/add">
				Tambah Business Office (BO) 
			</a>
		</li>
	</ul>
</div>
		...
		<div class="portlet"> 
			 <div class="portlet box blue">
				<div class="portlet-title">
					<div class="caption">
						Form Tambah Business Office (BO)
					</div>
					<div class="actions">
							...
							<div class="col-md-12">
								<table width="100%">
									<tr>
										...
										<td width="30%">Nama BO<span class="required">*</span></td>
										<td width="30%">Rek Tabungan<span class="required">*</span></td>
										<td width="20%">Cabang<span class="required">*</span></td>
									</tr>
									<tr>
										<td width="20%"><input type="text" class="easyui-textbox" name="office_code" id="office_code" value="<?php echo $data['office_code']; ?>" style="width: 100%" autofocus/></td>
										<td width="30%"><input type="text" class="easyui-textbox" name="office_name" id="office_name" value="<?php echo $data['office_name']; ?>" style="width:100%"/></td>
										<td width="30%"><?php echo form_dropdown('savings_account_id', $acctsavingsaccount, set_value('savings_account_id',$data['savings_account_id']),'id="savings_account_id" class="easyui-combobox" style="width:100%"');?></td>
										<td width="20%"><?php echo form_dropdown('branch_id', $corebranch, set_value('branch_id',$data['branch_id']),'id="branch_id" class="easyui-combobox" style="width:100%"');?></td>
									</tr>
								</table>
							</div>
						</div>
								...
								<table width="100%">
									...
									<tr>
										<td colspan="4" align="right">
											<input type="button" name="add2" id="buttonAddArrayAcctjournalVoucher" value="Add" class="btn green-jungle" title="Simpan Data" onClick="addCoreDusun();">
										</td>
									</tr>
								</table>
							</div>
				...
				<div class="form-body">
					<table class="table table-striped table-bordered table-hover table-full-width">
						...
						<tbody>
							...
							$datadusun = $this->session->userdata('addcoredusun-'.$sesi['unique']);
								...
								foreach ($datadusun as $key => $val) {
									echo "
										<tr>
											...
											<td><a href='".$this->config->item('base_url').'office/delete-dusun/'.$val['record_id']."' class='btn default btn-xs red', onClick='javascript:return confirm(\"apakah yakin ingin dihapus ?\")'>
