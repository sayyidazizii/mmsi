	...
	$(document).ready(function(){
        $("#Save").click(function(e){
			...
			$.ajax({
					type: 'post',
					data: dataString,       
					url: '<?php echo base_url();?>salary-payments/process-add',
					success: function (results) {
						
						idlastpayment = results;
						if(idlastpayment != 'error'){
							openInNewTab(base_url+"salary-payments/print-note/"+idlastpayment);
						}
						document.location = base_url+"salary-payments/add";
					}
			});
		});
    });
...
<?php
	...
	if($accountcredit['member_account_receivable_status'] == 1) {
?>
		<div class='alert alert-danger alert-dismissable'>  
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
			Anggota Diblokir dari Potong Gaji
		</div>
<?php 	
	} 
	if($accountcredit['member_account_receivable_amount'] > (5 * $accountcredit['member_mandatory_savings'])) {
?>
		<div class='alert alert-danger alert-dismissable'>  
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
			Potong Gaji Anggota Melebihi Limit
		</div>
<?php } ?>
<script>
						...
						<div class="row">
							...
							<div class="row">
								<div class="col-md-12" style='text-align:right'>
									...
									<?php if($accountcredit['member_account_receivable_status'] == 1 || $accountcredit['member_account_receivable_amount'] > (5 * $accountcredit['member_mandatory_savings'])) { ?>
...
$(document).ready(function() {
    table = $('#akadtable').DataTable({ 
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo site_url('salary-payments/akad-list-tunai')?>",
            "type": "POST"
        },
        "columnDefs": [
        { 
